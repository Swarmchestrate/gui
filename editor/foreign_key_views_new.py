...
class DialogBasedOneToOneFieldView(View):
    table_name: str
...
class DialogBasedOneToManyFieldView(View):
    table_name: str
    resource_type: str
...
class EditorForeignKeyFieldView(View):
    ...
    dialog_based_one_to_many_field_view_class: DialogBasedOneToManyFieldView
    ...
    def get(self, request, *args, **kwargs):
        # Determine if the column used for a FK reference
        # is specified in the definition or if it's a virtual
        # property.
        api_client = ApiClient()
        api_client.initialise_openapi_spec()
        openapi_spec = api_client.openapi_spec
        definition = openapi_spec.get_definition(self.table_name)
        if self.fk_column_name in definition.properties:
            fk_table_name = definition.get_foreign_key_table_name_for_column(
                self.fk_column_name
            )
